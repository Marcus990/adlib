#!/usr/bin/env bash
# Package the release binary as build/AdLib.app (double-clickable; no terminal needed).
...
set -euo pipefail
cd "$(dirname "$0")/.."
[ -x target/release/adlib ] || { echo "build first: CARGO_BUILD_JOBS=2 cargo build --release"; exit 1; }
APP="build/AdLib.app"
rm -rf "$APP"; mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp target/release/adlib "$APP/Contents/MacOS/adlib"
...
<plist version="1.0"><dict>
  <key>CFBundleName</key><string>AdLib</string>
  <key>CFBundleDisplayName</key><string>AdLib</string>
  <key>CFBundleIdentifier</key><string>dev.adlib.app</string>
  <key>CFBundleExecutable</key><string>adlib</string>
  ...
  <key>NSMicrophoneUsageDescription</key><string>AdLib listens to the presenter to choose the picture on screen. Audio is transcribed on this Mac.</string>
</dict></plist>
PLIST
