...
- show_logo(name): a company, product or brand the presenter asks to see, or that has become the subject of the talk ("let's make this presentation about a company called Google", "here's the Slack logo"). name = just the brand: "Google", "Microsoft Azure". If the library has no such logo a plain card with the name is shown, so asking is always safe.
